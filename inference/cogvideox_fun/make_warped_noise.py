...
# Use pre-cloned CommonSource (~/.rp/git/CommonSource) and pre-installed packages.
# Bypasses rp's auto-installer which requires sudo on restricted machines.
_common_source_path = os.path.expanduser("~/.rp/git/CommonSource")
if _common_source_path not in sys.path:
    sys.path.insert(0, _common_source_path)
...
def main(video: str, output_folder: str):
    ...
    video = rp.resize_list(
        video, length=72
    )  # Stretch or squash video to 72 frames (for training)
    ...
    output = nw.get_noise_from_video(
        ...
        visualize=False,  # Disabled: rp.save_video_mp4 hits a bitrate overflow with av library
    ...
    # Note: rp.save_video_mp4 with video_bitrate='max' overflows the av library's int codec on this platform.
    # The preview video is not needed by Pass 2 inference — only noises.npy matters.
