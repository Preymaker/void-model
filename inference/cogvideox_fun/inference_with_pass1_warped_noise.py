...
def generate_warped_noise_from_video(video_path, output_dir):
    ...
    try:
        result = subprocess.run(
            ...
            capture_output=True,
            text=True,
            timeout=3600,  # 60 minute timeout (optical flow is CPU-bound, can take 10-30 min)
        )
...
def load_and_resize_warped_noise(noise_path, target_shape, device, dtype):
    ...
    logger.info(
        f"Loaded noise: {warped_noise_np.shape}, dtype: {warped_noise_np.dtype}"
    )
        ...
        for t in range(latent_T):
            ...
            for c in range(frame.shape[2]):
                ...
                channel_resized = cv2.resize(
                    channel, (latent_W, latent_H), interpolation=cv2.INTER_LINEAR
                )
...
def main():
    ...
    parser.add_argument(
        "--pass1_dir",
        type=str,
        default="./pass1_outputs",
        help="Directory containing pass 1 videos",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="./inference_with_warped_noise",
    ...
    parser.add_argument(
        "--model_name",
        type=str,
        default="./CogVideoX-Fun-V1.5-5b-InP",
    ...
    # Video settings
    parser.add_argument(
        "--max_video_length", type=int, default=197, help="Maximum video length"
    )
    parser.add_argument(
        "--temporal_window_size", type=int, default=85, help="Temporal window size"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="./pass1_warped_noise_cache",
    ...
    # Misc
    parser.add_argument(
        "--use_quadmask", action="store_true", default=True, help="Use quadmask format"
    )
    ...
    if param_name in state_dict and param_name in transformer.state_dict():
        if state_dict[param_name].size(1) != transformer.state_dict()[param_name].size(
            1
        ):
    ...
    vae = (
        AutoencoderKLCogVideoX.from_pretrained(args.model_name, subfolder="vae")
        .to(weight_dtype)
        .to(device)
    )
    ...
    for idx, video_name in enumerate(video_names, 1):
        ...
        try:
            # Find pass 1 video
            logger.info("  [a] Finding pass 1 video...")
            ...
            logger.info("  [b] Generating/loading warped noise...")

            if args.skip_noise_generation:
                video_stem = Path(pass1_video_path).stem
                noise_file = (
                    Path(args.warped_noise_cache_dir) / video_stem / "noises.npy"
                )
                if not noise_file.exists():
                ...
                if warped_noise_path is None:
                    logger.error(
                        f"  ✗ Failed to generate warped noise for {video_name}"
                    )
                    failed += 1
                    continue

            # Load input video & mask
            logger.info("  [c] Loading input video and mask...")
            input_video, input_video_mask, prompt, _ = get_video_mask_input(
                ...
                use_trimask=False,
                use_quadmask=args.use_quadmask,
            )
            ...
            # Run inference
            logger.info("  [d] Running inference...")
            generator = torch.Generator(device=device).manual_seed(args.seed)

            with torch.no_grad():
                output = pipeline(
                    prompt,
                    num_frames=args.temporal_window_size,
                    negative_prompt="",
                    height=args.height,
                    width=args.width,
                    generator=generator,
            ...
            output_path = (
                Path(args.output_dir) / f"{video_name}_warped_noise_inference.mp4"
            )
            imageio.mimsave(
                output_path,
                video_np,
